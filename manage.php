...
ORDER BY user_building.name,user_room.short_name
		...
		if($roughlist) foreach($roughlist as $oneroom){
			...
			$oneroom['thedate']='';
			$oneroom['outcome']='';
			$oneroom['inspectme']="<button style=\"background-color: #4CAF50;\" onClick=\"window.location.href='/rooms/inspect.php?status=new&room_id=$oneroom[room_id]&source=manage&faculty=$faculty'\">Inspect</button>";
